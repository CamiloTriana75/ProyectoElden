...
export const Contact: React.FC = () => {
  ...
  // Cargar mensajes si es empleado
  useEffect(() => {
    if (user?.role === 'employee' || user?.role === 'admin') {
      db.getMessages().then((msgs) => setMessages((msgs as Message[]).sort((a: Message, b: Message) => b.createdAt.localeCompare(a.createdAt))));
    }
  }, [user]);
  ...
  const markAsRead = async (msg: Message) => {
    ...
    const updated: Message = { ...msg, status: 'read' };
  ...
  if (user?.role === 'employee' || user?.role === 'admin') {
    return (
      <div className="p-8 max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold text-white mb-8 text-center">Mensajes</h1>
        <div className="flex gap-8">
          {/* Panel izquierdo: filtros y lista de mensajes */}
          <div className="w-1/3 min-w-[280px]">
            ...
            <div className="bg-white/10 rounded-xl overflow-y-auto max-h-[400px]">
          ...
          <div className="flex-1 flex flex-col justify-between bg-white/10 rounded-xl p-8 min-h-[400px]">
            {selectedMessage ? (
              <>
                <div>
            ...
            <div className="mt-8 border-t border-gray-700 pt-4 flex flex-col items-center">
              <div className="text-green-200 font-bold">Nuestras Redes:</div>
        ...
        <div className="text-center text-cyan-200 mt-8 font-bold">Contactos<br />copyright @2025</div>
      </div>
    );
  }
